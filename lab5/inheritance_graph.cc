...
void print_error_message(const std::string &filename, const int &line,
...
size_t find_class_redefinitions() {
  std::set<std::string> classes_set{"Object", "IO",   "Int",
                                    "String", "Bool", "SELF_TYPE"};
  size_t redefinitions = 0;
  ...
  for (const auto root : ast_roots) {
    ...
    for (int i = 0; i < classes->len(); ++i) {
      ...
      if (classes_set.find(name) != classes_set.end()) {
        print_error_message(current_class->filename->get_string(),
                            current_class->line_number,
                            "redifinition of class " + name);
        ++redefinitions;
      } else
...
std::set<std::string> make_types_table() {
  std::set<std::string> types_table{"Object", "IO",   "Int",
                                    "String", "Bool", "SELF_TYPE"};
...
void semant() {
  ...
  for (const auto &node : extra_classes) {
    for (const auto &kid : node->kids) {
      ...
      print_error_message(filename, line,
                          "class \"" + kid->class_name +
                              "\" inherits from class \"" + class_name +
                              "\" that not defined");
    }
  }
  if (faults_attend)
